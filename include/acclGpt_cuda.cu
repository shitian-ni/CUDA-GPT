...
__device__ int d_g_ang2[ROW][COL];
...
__device__ int d_cuda_global_fwinpat_count[2];
__device__ double d_cuda_global_fwinpat_dnn[2];
...
void*  d_g_ptr;
void*  d_g_can1_ptr;void*  d_g_nor1_ptr;void*  d_g_ang1_ptr;void*  d_g_ang2_ptr;void* d_sHoG1_ptr;void* d_sHoG2_ptr;
...
void* d_cuda_global_fwinpat_count_ptr;
void* d_cuda_global_fwinpat_dnn_ptr;
...
void cuda_init_parameter(){
	...
	gpuErrchk( cudaGetSymbolAddress(&d_cuda_global_fwinpat_count_ptr,d_cuda_global_fwinpat_count));
	gpuErrchk( cudaGetSymbolAddress(&d_cuda_global_fwinpat_dnn_ptr,d_cuda_global_fwinpat_dnn));
	...
	gpuErrchk( cudaGetSymbolAddress(&d_g_ang2_ptr,d_g_ang2));
...
void copy_initial_parameters(double gk[ROW][COL],double g_can2[ROW][COL],int g_ang2[ROW][COL],double H1[ROW_H1][COL_H1],double H2[ROW_H2][COL_H2],double H3[ROW_H3][COL_H3],
...
int coor[(2 * ROW - 1) * (2 * COL - 1)][2]){
	...
	cudaMemcpy(d_g_ang2_ptr, g_ang2, ROW * COL * sizeof(int), cudaMemcpyHostToDevice);
...
__global__ void cuda_global_fwinpat() {
	int tx = threadIdx.x;
    int ty = threadIdx.y;
    int tid = ty * blockDim.x + tx;
	int x = blockIdx.x*blockDim.x + threadIdx.x;
    ...
    // }
    // if(x==30 && y==60){
    // 	printf("%d %d\n", d_g_ang1[y][x],d_g_ang2[y][x]);
    // }
  	int margine = 4;
    double minInit = sqrt((double)((ROW - 2 * margine) * (ROW - 2 * margine) + (COL - 2 * margine) * (COL - 2 * margine)));

    bool condition1 = ((y >= MARGINE) && (x >= MARGINE) && (y < ROW-MARGINE) && (x < COL-MARGINE) && d_g_ang1[y - MARGINE][x - MARGINE] != -1);
    bool condition2 = ((y >= MARGINE) && (x >= MARGINE) && (y < ROW-MARGINE) && (x < COL-MARGINE) && d_g_ang2[y - MARGINE][x - MARGINE] != -1);
    int angcode1 = 0;
    if(condition1)angcode1 = d_g_ang1[y][x];
    ...
    double min_2 = condition2*minInit;
    int angcode2 = 0;
    if(condition2)angcode2 = d_g_ang2[y][x];
    for (int y1 = 0 ; condition2 && y1 < (2 * ROW - 1) * (2 * COL - 1) ; y1++) {
        if (y + d_coor[y1][0] < 0 || y + d_coor[y1][0] >= ROW || x + d_coor[y1][1] < 0 || x + d_coor[y1][1] >= COL ) continue;
        if (d_g_ang1[y + d_coor[y1][0]][x + d_coor[y1][1]] != angcode2) continue;
        delta_2 = d_ndis[y1];
        // printf("y1 = %d nn1 = %f \n", y1, ndis[y1]);
        if (delta_2 < min_2) min_2 = delta_2;
        break;
	...
	customAdd(sdata_int[0],d_cuda_global_fwinpat_count);
	customAdd(sdata_int[1],d_cuda_global_fwinpat_count+1);
	customAdd(sdata_double[0],d_cuda_global_fwinpat_dnn);
...
double cuda_fwinpat(int g_ang2[ROW][COL]){
	...
	cudaMemset(d_cuda_global_fwinpat_count_ptr,0,2*sizeof(int));
	// cudaMemcpy(d_g_ang2_ptr,g_ang2,(ROW)*(COL)*sizeof(int),cudaMemcpyHostToDevice);

	cuda_global_fwinpat<<<numBlock, numThread>>>();
	gpuErrchk( cudaDeviceSynchronize() );
    gpuErrchk( cudaThreadSynchronize() ); // Checks for execution error
	gpuErrchk( cudaPeekAtLastError() ); // Checks for launch error

	int cuda_global_fwinpat_count[2];
	double cuda_global_fwinpat_dnn[2];
	cudaMemcpy(cuda_global_fwinpat_count, d_cuda_global_fwinpat_count_ptr, 2*sizeof(int), cudaMemcpyDeviceToHost);
	cudaMemcpy(cuda_global_fwinpat_dnn, d_cuda_global_fwinpat_dnn_ptr, 2*sizeof(double), cudaMemcpyDeviceToHost);
	double dnn1 = cuda_global_fwinpat_dnn[0] / cuda_global_fwinpat_count[0];
	double dnn2 = cuda_global_fwinpat_dnn[1] / cuda_global_fwinpat_count[1];
    return (dnn1 + dnn2)/2.0;
}